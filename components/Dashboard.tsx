...
const Dashboard: React.FC = () => {
  ...
  // --- Installment Logic ---
  // Generate "Virtual" expenses for ALL installments that fall within the current billing period OR are in the past.
  // This allows seeing upcoming installments for *this* period in the expense list.
  const installmentExpenses = useMemo(() => {
    const virtualExpenses: Expense[] = [];
    
    // We want to list all installments up to the END of the current billing period.
    // This effectively treats "future" days in the current cycle as "viewable" expenses.
    const cutoffDate = currentPeriod.end;

    paymentPlans.forEach(plan => {
        // Parse start date in Local Time to avoid timezone offsets
        ...
        for (let i = 0; i < plan.totalInstallments; i++) {
            ...
            // If the installment date is on or before the cycle end date, include it.
            // Using setHours to ignore time component in comparison just in case
            pDate.setHours(0,0,0,0);
            const cutoff = new Date(cutoffDate);
            cutoff.setHours(23,59,59,999);

            if (pDate <= cutoff) {
                // Formatting date string as YYYY-MM-DD manually to respect local time
                ...
                virtualExpenses.push({
                    ...
                    category: plan.title, // Use plan TITLE as category name for Reports
  ...
  // Merge Real Expenses with Virtual Installments
  ...
  const currentPeriodStats = useMemo(() => {
    // Filter expenses strictly within the current billing cycle
  ...
  // Logic for Budgets View (Billing Period)
  const budgetStats = useMemo(() => {
    // Count expenses in current period
    const periodExps = currentPeriodStats.expenses;

    // First map standard categories
    ...
    // Calculate unmatched expenses (debts/installments typically fall here)
    ...
    // Find the managed "Borçlar & Planlar" category
    const debtCategoryIndex = stats.findIndex(s => s.id === '9' || s.name === 'Borçlar & Planlar');

    if (debtCategoryIndex !== -1) {
        // Add unmatched amount to the managed category
        stats[debtCategoryIndex].spent += debtSpent;
    } else if (debtSpent > 0) {
        // Fallback: If user deleted the managed category, show virtual one
        stats.push({
            id: 'debt-virtual',
  ...
  const periodData = useMemo(() => {
    ...
    if (reportPeriod === 'Haftalık') {
        ...
        // Use Billing Cycle Period
        startDate = new Date(currentPeriod.start);
        endDate = new Date(currentPeriod.end);
        
        // Previous period is roughly one month back
        prevStartDate = new Date(startDate);
        prevStartDate.setMonth(prevStartDate.getMonth() - 1);
        prevEndDate = new Date(startDate);
        prevEndDate.setDate(prevStartDate.getDate() - 1); // Day before start of current

        // Recalculate days count for chart to match billing cycle length
    ...
    if (reportPeriod === 'Yıllık') {
        ...
        for (let i = 0; i < daysCount; i++) {
            ...
            // Correctly format local date to YYYY-MM-DD for comparison
  ...
  // Calculate Category Stats specifically for the selected Report Period
  ...
  const renderHomeView = () => (
    <>
        {/* Brand Logo Header */}
        {activeTab === 'home' && (
             <div className="flex flex-col items-center justify-center pt-8 pb-8 animate-in slide-in-from-top-4 fade-in duration-500">
                <div className="flex items-center gap-3">
                    <div className="relative size-12 bg-gradient-to-tr from-blue-600 to-blue-400 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20 transform -rotate-3">
                        <span className="material-symbols-outlined text-white text-3xl">account_balance_wallet</span>
                    </div>
                    <h1 className="text-3xl font-[900] tracking-tighter text-zinc-900 dark:text-white">
        ...
        {(() => {
            switch(homeViewMode) {
                case 'plans':
                    return <PaymentPlan />;
                case 'subscriptions':
                    return <Subscriptions isEmbedded={true} />;
                case 'expenses':
                default:
                    return (
                        /* Expenses List View */
                        <>
                            ...
                            >
                                ...
                                <p className="text-sm font-medium leading-normal">
                                    {filterDateStart || filterDateEnd ? 'Tarih Seçili' : 'Tarih Aralığı'}
                                </p>
                            ...
                            {hasActiveFilters && (
                                <button 
                                    ...
                                    className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400"
                                >
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <input 
                                                    ...
                                                    className="bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-gray-600 rounded-lg p-2 text-sm dark:text-white focus:ring-primary focus:border-primary"
                                                />
                                            </div>
                                            <div className="flex flex-col gap-1">
                                    ...
                                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                                        <button
                                            ...
                                            className={`whitespace-nowrap px-4 py-2 rounded-full text-sm font-medium border transition-colors ${
                                                ...
                                                : 'bg-white dark:bg-zinc-800 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300'
                                            }`}
                                        >
                                            Tümü
                                        </button>
                            ...
                            {/* Summary Card (Budget & Balance) */}
                            <div className="px-4 py-2">
                                <div 
                                    onClick={() => setActiveTab('budgets')}
                                    className="flex flex-col rounded-3xl shadow-md bg-white dark:bg-zinc-800 dark:border dark:border-white/5 p-6 relative overflow-hidden transition-all group cursor-pointer hover:shadow-lg hover:scale-[1.01]"
                                >
                                    ...
                                    <div className="relative z-10 mb-6">
                                        <p className="text-zinc-500 dark:text-zinc-400 text-xs font-bold uppercase tracking-wide mb-1">
                                            Kalan Bakiye
                                        </p>
                                        ...
                                        <p className="text-[10px] text-gray-400 mt-1">
                                            Dönem: {formatDateShort(currentPeriod.start)} - {formatDateShort(currentPeriod.end)}
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-4 relative z-10">
                                        ...
                                        <div className="flex-1 bg-gray-50 dark:bg-zinc-900/50 p-3 rounded-2xl border border-gray-100 dark:border-white/5 relative group/budget">
                                            <div className="flex items-center justify-between mb-1">
                                                ...
                                                <button 
                                                    onClick={(e) => { e.stopPropagation(); openBudgetEdit(); }} 
                                                    className="opacity-0 group-hover/budget:opacity-100 transition-opacity text-primary hover:text-primary/80"
                                                >
                                        ...
                                        <div className="flex-1 bg-gray-50 dark:bg-zinc-900/50 p-3 rounded-2xl border border-gray-100 dark:border-white/5">
                                            ...
                                            <p className="text-lg font-bold text-red-600 dark:text-red-400">
                                                -₺{currentPeriodTotal.toLocaleString('tr-TR', { maximumFractionDigits: 2 })}
                                            </p>
                                        </div>
                                ...
                                {sortedDates.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center py-12 px-6 text-center animate-in fade-in zoom-in-95">
                                        ...
                                        <p className="text-gray-500 dark:text-gray-400 text-sm max-w-xs mb-8">
                                            İlk harcamanı ekleyerek bütçeni yönetmeye başla. İstersen fiş tarayabilir veya elle ekleyebilirsin.
                                        </p>
                                        <button 
                                            onClick={() => navigate('/manual')}
                                            className="px-6 py-3 rounded-xl bg-primary text-white font-bold shadow-lg shadow-primary/25 hover:bg-primary/90 transition transform active:scale-95"
                                        >
                                            İlk Harcamanı Ekle
                                        </button>
                                    </div>
                                    ...
                                    <div key={date}>
                                        <h3 className="text-zinc-900 dark:text-zinc-100 text-lg font-bold leading-tight tracking-[-0.015em] pt-6 pb-3 sticky top-[60px] z-0">
                                            {getDayLabel(date)}
                                        </h3>
                                        <div className="flex flex-col gap-3">
                                            {groupedExpenses[date].map(expense => {
                                                ...
                                                // Highlight future installments in this period
                                                const isFuture = new Date(expense.date) > new Date();
                                                
                                                return (
                                                    <SwipeableListItem 
                                                        key={expense.id} 
                                                        onDelete={() => !isVirtual && removeExpense(expense.id)}
                                                        className={`shadow-sm ${isVirtual ? 'border-l-4 border-indigo-500' : ''} ${isFuture ? 'opacity-70 bg-gray-50 dark:bg-zinc-800/50' : ''}`}
                                                        disabled={isVirtual}
                                                    >
                                                        <div className="flex items-center gap-4 p-3 min-h-[72px] justify-between border border-transparent hover:border-primary/20 dark:hover:border-primary/20 transition-all cursor-pointer">
                                                            <div className="flex items-center gap-4">
                                                                <div 
                                                                    className="text-zinc-900 dark:text-zinc-100 flex items-center justify-center rounded-xl shrink-0 size-12"
                                                                    style={{ backgroundColor: isVirtual ? `${catDetails.color}20` : `${catDetails.color}20`, color: catDetails.color }}
                                                                >
                                                                    <span 
                                                                ...
                                                                <div className="flex flex-col justify-center">
                                                                    ...
                                                                    <p className="text-zinc-500 dark:text-zinc-400 text-sm font-normal leading-normal line-clamp-2">
                                                                        {isVirtual ? expense.category : expense.category}
                                                                        {isFuture && <span className="text-xs text-orange-500 ml-2 font-bold">(Bekleyen)</span>}
                                                                    </p>
                                                                </div>
                                                            </div>
            ...
            <div className="bg-white dark:bg-zinc-800 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-gray-700/50">
                <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="text-[10px] bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded text-gray-500">
                        {formatDateShort(currentPeriod.start)} - {formatDateShort(currentPeriod.end)}
                    </span>
                </div>
                <h1 className="text-4xl font-black text-gray-900 dark:text-white mb-1">
                    ₺{remainingTotal.toLocaleString('tr-TR', { maximumFractionDigits: 0 })}
                </h1>
                    ...
                    <div className="flex flex-col group/editbudget">
                        <div className="flex items-center gap-2">
                             ...
                             <button 
                                ...
                                className="text-primary hover:text-primary/80 transition p-1 rounded-full hover:bg-primary/10"
                             >
                                <span className="material-symbols-outlined text-sm font-bold">add</span>
                             </button>
                        </div>
  ...
  const renderReportsView = () => {
    ...
    return (
      <div className="px-4 pb-20 pt-2 animate-in slide-in-from-right-4 fade-in duration-300 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white dark:bg-zinc-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/50 flex flex-col gap-1">
                  ...
                  <span className="text-xl font-black text-gray-900 dark:text-white">
                      ₺{(currentTotal / (reportPeriod === 'Haftalık' ? 7 : 30)).toLocaleString('tr-TR', { maximumFractionDigits: 2 })}
                  </span>
              </div>
          </div>
          ...
          {/* Top Categories - FILTERED BY PERIOD */}
          <div>
              ...
              <div className="flex flex-col gap-3">
                  {reportCategoryStats.length === 0 ? (
                    ...
                    reportCategoryStats.map(([catName, amount]) => {
                      ...
                      // Use currentTotal (filtered) for percentage calculation
                          ...
                          <div key={catName} className="bg-white dark:bg-zinc-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/50 flex items-center justify-between">
                             <div className="flex items-center gap-3">
                                 <div 
                                    ...
                                    style={{ backgroundColor: `${catDetails.color}20`, color: catDetails.color }}
                                 >
  ...
  const renderSettingsView = () => (
      <div className="px-4 pb-20 pt-2 animate-in slide-in-from-right-4 fade-in duration-300 flex flex-col gap-6">
           ...
           <div className="flex flex-col gap-2">
               ...
               <div className="bg-white dark:bg-zinc-800 rounded-xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700/50">
                   <button 
                       ...
                       className="flex w-full items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition border-b border-gray-100 dark:border-gray-700/50 last:border-0"
                   >
                       <div className="flex items-center gap-3">
                           <div className="size-10 rounded-lg bg-gray-100 dark:bg-zinc-700 flex items-center justify-center text-zinc-900 dark:text-white">
           ...
           <div className="flex flex-col gap-2">
               ...
               <div className="bg-white dark:bg-zinc-800 rounded-xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700/50">
                   {/* Statement Date (Hesap Kesim Tarihi) */}
                   <div className="flex w-full items-center justify-between p-4 border-b border-gray-100 dark:border-gray-700/50">
                       ...
                       <select 
                            ...
                            className="bg-transparent border-none text-right font-bold text-primary focus:ring-0 cursor-pointer"
                       >
                            {[...Array(31)].map((_, i) => (
                   ...
                   <button 
                       ...
                       className="flex w-full items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition border-b border-gray-100 dark:border-gray-700/50"
                   >
                       <div className="flex items-center gap-3">
                           <div className="size-10 rounded-lg bg-gray-100 dark:bg-zinc-700 flex items-center justify-center text-zinc-900 dark:text-white">
                   ...
                   <div 
                      ...
                      className="flex w-full items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition border-b border-gray-100 dark:border-gray-700/50 cursor-pointer"
                   >
                       <div className="flex items-center gap-3">
                           <div className="size-10 rounded-lg bg-gray-100 dark:bg-zinc-700 flex items-center justify-center text-zinc-900 dark:text-white">
           ...
           <div className="flex flex-col gap-2">
               ...
               <div className="bg-white dark:bg-zinc-800 rounded-xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700/50">
                   ...
                   <button 
                       ...
                       className="flex w-full items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition"
                   >
                       <div className="flex items-center gap-3">
           ...
           <div className="flex flex-col gap-2">
               ...
               <div className="bg-white dark:bg-zinc-800 rounded-xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700/50">
                    <button 
                        ...
                        className="flex w-full items-center gap-3 p-4 hover:bg-red-50 dark:hover:bg-red-900/10 transition group"
                    >
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col bg-background-light dark:bg-background-dark group/design-root overflow-x-hidden font-display">
      ...
      {/* Top App Bar */}
      <div className="sticky top-0 z-30 bg-background-light dark:bg-background-dark/95 backdrop-blur-sm pt-4 border-b border-gray-200 dark:border-gray-800 transition-all">
        <div className="flex items-center bg-transparent p-4 pb-2 justify-between">
            {activeTab !== 'home' && (
                 <button onClick={() => setActiveTab('home')} className="text-gray-800 dark:text-gray-200 flex size-10 shrink-0 items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition">
            ...
            {activeTab === 'home' ? (
                <div className="flex-1 flex justify-center">
                    <div className="flex items-center bg-gray-100 dark:bg-zinc-800 rounded-lg p-1">
                        <button 
                            onClick={() => setHomeViewMode('expenses')}
                            className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${homeViewMode === 'expenses' ? 'bg-white dark:bg-zinc-600 shadow-sm text-black dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                        >
                            Harcamalar
                        </button>
                        <button 
                            onClick={() => setHomeViewMode('plans')}
                            className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${homeViewMode === 'plans' ? 'bg-white dark:bg-zinc-600 shadow-sm text-black dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                        >
                            Planlar
                        </button>
                        <button 
                            onClick={() => setHomeViewMode('subscriptions')}
                            className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all ${homeViewMode === 'subscriptions' ? 'bg-white dark:bg-zinc-600 shadow-sm text-black dark:text-white' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}
                        >
                            Abonelikler
                        </button>
                    </div>
                ...
                <h1 className="text-zinc-900 dark:text-zinc-100 text-2xl font-bold leading-tight tracking-tighter flex-1 text-center pr-10">
                    {activeTab === 'reports' ? 'Harcama Analizi' : 
                     activeTab === 'budgets' ? 'Bütçelerim' :
                     'Ayarlar'}
                </h1>
            )}
            ...
            {activeTab === 'home' && (
               <button 
                 onClick={() => navigate('/categories')}
                 className="text-zinc-900 dark:text-zinc-100 flex size-10 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-zinc-800/50 border border-gray-200 dark:border-zinc-700 hover:bg-gray-200 dark:hover:bg-zinc-700 transition"
            ...
            {activeTab === 'reports' && (
               <div className="text-zinc-900 dark:text-zinc-100 flex size-12 shrink-0 items-center justify-end">
      ...
      {isFabOpen && (
        <div 
            className="fixed inset-0 bg-black/60 z-50 backdrop-blur-md transition-opacity duration-300 animate-in fade-in" 
            onClick={() => setIsFabOpen(false)}
        />
      )}

      {/* FAB Menu */}
      <div className={`fixed bottom-24 left-1/2 -translate-x-1/2 flex flex-col gap-4 items-center z-50 pointer-events-none ${isFabOpen ? 'pointer-events-auto' : ''}`}>
         <button 
            onClick={() => {
                navigate('/manual');
                setIsFabOpen(false);
            }}
            className={`flex items-center gap-3 group w-48 bg-white dark:bg-zinc-800 p-2 pr-4 rounded-full shadow-xl border border-gray-100 dark:border-gray-700 transition-all duration-300 delay-75 transform ${isFabOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-8 opacity-0 scale-95'}`}
         >
         ...
         <button 
            ...
            className={`flex items-center gap-3 group w-48 bg-white dark:bg-zinc-800 p-2 pr-4 rounded-full shadow-xl border border-gray-100 dark:border-gray-700 transition-all duration-300 delay-50 transform ${isFabOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-8 opacity-0 scale-95'}`}
         >
             <div className="flex size-10 items-center justify-center rounded-full bg-indigo-500 text-white shadow-lg">
         ...
         <button 
            ...
            className={`flex items-center gap-3 group w-48 bg-white dark:bg-zinc-800 p-2 pr-4 rounded-full shadow-xl border border-gray-100 dark:border-gray-700 transition-all duration-300 transform ${isFabOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-8 opacity-0 scale-95'}`}
         >
             <div className="flex size-10 items-center justify-center rounded-full bg-primary text-white shadow-lg">
      ...
      <BottomNavbar 
        ...
        onAddClick={() => setIsFabOpen(!isFabOpen)}
        isOpen={isFabOpen}
      />
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 w-full max-w-xs shadow-2xl animate-in zoom-in-95 duration-200">
                ...
                <div className="relative mb-6">
                    ...
                    <input 
                        type="number" 
                        value={budgetToAdd} 
                        onChange={e => setBudgetToAdd(e.target.value)}
                        className="w-full bg-gray-50 dark:bg-zinc-800 border-none rounded-xl py-4 pl-12 pr-4 text-2xl font-bold text-center text-zinc-900 dark:text-white focus:ring-2 focus:ring-primary focus:bg-white dark:focus:bg-zinc-800 transition-all placeholder-gray-300 dark:placeholder-gray-700"
                        autoFocus
                        placeholder="0"
                    />
                </div>
                ...
                <div className="flex gap-3">
                    <button 
                        ...
                        className="flex-1 py-3 rounded-xl bg-gray-100 dark:bg-zinc-800 font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-zinc-700 transition"
                    >
                        İptal
                    </button>
                    <button 
                        onClick={handleSaveBudget} 
                        className="flex-1 py-3 rounded-xl bg-primary font-bold text-white hover:bg-primary/90 transition shadow-lg shadow-primary/20"
                    >
                        Ekle
                    </button>
                </div>
